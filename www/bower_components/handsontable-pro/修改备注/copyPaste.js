...
function CopyPastePlugin(instance) {
  ...
  this.getRangedData = function(ranges) {
    ...
    arrayEach(copyableRows, (row) => {
      ...
      arrayEach(copyableColumns, (column) => {
        if (settings.copyType !== 'name') {
          rowSet.push(instance.getCopyableData(row, column));
        } else {
          // debugger;
          let code = instance.getCopyableData(row, column);
          let prop = instance.colToProp(column);
          let settingsColumn = settings.columns[column];
          let record = {};
          let name;
          record[prop] = code;
          if (settingsColumn && settingsColumn.valueParser) {
            try {
              name = settingsColumn.valueParser(code, record);
            } catch (e) {
              name = code;
            }
          } else {
