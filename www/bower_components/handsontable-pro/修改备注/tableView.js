      ...
      if (event.shiftKey && actualSelection) {
        ...
        if (coords.row < 0 && coords.col >= 0 && !blockCalculations.column) {
          ...
          if (leftClick || (rightClick && doNewSelection)) {
            ...
            let selectedRow = (selected && selected[0]) || 0;
            selection.setRangeStartOnly(new CellCoords(selectedRow, coords.col));
            selection.setRangeEnd(new CellCoords(selectedRow, coords.col), false);
